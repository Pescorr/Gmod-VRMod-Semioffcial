...
-- Delegate to magbone's API if available (includes user customization + overrides)
-- Fallback to hardcoded keywords if magbone is not loaded
local function IsMagBone(boneName)
    if vrmod.IsMagazineBone then
        return vrmod.IsMagazineBone(boneName)
    end
    -- Fallback (should not reach here if magbone dependency enforced)
    local lower = string.lower(boneName)
    for _, kw in ipairs({"mag", "clip", "magazine", "ammo", "cylin", "shell"}) do
...
hook.Add("VRMod_PreRender", "VRRealMech_ReloadAnim", function()
    ...
    if vrmod.RealMech._magboneMissing then return end
