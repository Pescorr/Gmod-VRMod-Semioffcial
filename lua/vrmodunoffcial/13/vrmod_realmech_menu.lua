--[[
    ...
    Adds "RealMech" tab to the nonVRGunSheet (alongside Foregrip and Magazine tabs).
]]
...
hook.Add("VRMod_Menu", "addsettings_realmech", function(frame)
    ...
    local scroll = vgui.Create("DScrollPanel", frame.nonVRGunSheet)
    frame.nonVRGunSheet:AddSheet("RealMech", scroll, "icon16/wrench.png")
    ...
    resetBtn:SetText(VRModL and VRModL("btn_restore_defaults", "Restore Default Settings") or "Restore Default Settings")
    resetBtn.DoClick = function()
        if VRModResetCategory then
            VRModResetCategory("realmech")
        else
            -- Fallback: reset manually if VRModResetCategory not available
            RunConsoleCommand("vrmod_unoff_realmech_enable", "1")
            RunConsoleCommand("vrmod_unoff_realmech_slide_enable", "1")
            RunConsoleCommand("vrmod_unoff_realmech_trigger_enable", "1")
            RunConsoleCommand("vrmod_unoff_realmech_bullet_enable", "1")
            RunConsoleCommand("vrmod_unoff_realmech_hammer_enable", "1")
            RunConsoleCommand("vrmod_unoff_realmech_selector_enable", "1")
            ...
            RunConsoleCommand("vrmod_unoff_realmech_slide_grab_enable", "1")
            ...
            RunConsoleCommand("vrmod_unoff_realmech_reload_idle_return", "1")
            RunConsoleCommand("vrmod_unoff_realmech_debug", "0")
        end
        if VRModL then
            chat.AddText(Color(0, 255, 0), VRModL("msg_realmech_reset", "RealMech settings reset to defaults."))
        else
