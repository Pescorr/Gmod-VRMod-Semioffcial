-- function vrmod_character_type2_lua()
--     g_VR = g_VR or {}
--     g_VR.characterYaw = 0
...
--         -- 0: Original (scale=0), 1: Alternative (position offset), 2: Hide all except arms/hands
--         CreateClientConVar("vrmod_hide_head", "0", true, FCVAR_ARCHIVE, "Hide player's head in VR (0=Scale, 1=Offset, 2=Hide Body)", 0, 2)
--     end
...
--             armHandBones = {} -- List to store arm/hand bone IDs
--         }
...
--         -- Populate armHandBones list
--         local armHandKeywords = {"Clavicle", "UpperArm", "Forearm", "Hand", "Wrist", "Ulna", "Finger"}
--         for boneID = 0, cm:GetBoneCount() - 1 do
--             local boneName = cm:GetBoneName(boneID)
--             for _, keyword in ipairs(armHandKeywords) do
--                 if string.find(boneName, keyword) then
--                     characterInfo[steamid].armHandBones[boneID] = true
--                     break
--                 end
--             end
--         end
...
--     -- Helper function from vrmod_character-lua-.txt
--     local function HideBoneAndChildren(ply, boneID, hide)
--         if not ply:IsValid() or not ply:GetBoneMatrix(boneID) then return end
--         local scaleVec = hide and zeroVec or Vector(1, 1, 1)
--         ply:ManipulateBoneScale(boneID, scaleVec)
...
--         -- Apply scaling/positioning only in first person for modes 1 and 2
--         if isFirstPerson and hideHeadSetting ~= 0 then
--             local charInfo = characterInfo[steamid]
--             if charInfo and charInfo.bones then
--                 -- Alternative head hiding (position offset)
--                 if hideHeadSetting == 1 then
--                     local headBoneID = charInfo.bones.b_head
--                     if headBoneID then
--                         -- Store original state if not already stored
--                         if not charInfo.originalHeadPosScale then
--                             local headMatrix = ply:GetBoneMatrix(headBoneID)
--                             if headMatrix then
--                                 charInfo.originalHeadPosScale = {
--                                     pos = headMatrix:GetTranslation(),
--                                     scale = Vector(1, 1, 1) -- Assuming default scale is 1
--                                 }
--                             end
--                         end

--                         -- Apply offset (example, needs refinement based on original intent)
--                         ply:ManipulateBonePosition(headBoneID, Vector(0, 0, -1000)) -- Move way back
--                         ply:ManipulateBoneScale(headBoneID, zeroVec)
--                     end
--                 elseif hideHeadSetting == 2 then
--                     -- Hide all except arms/hands
--                     local armHandBones = charInfo.armHandBones
--                     for boneID = 0, ply:GetBoneCount() - 1 do
--                         if not armHandBones[boneID] then
--                             ply:ManipulateBoneScale(boneID, zeroVec)
--                         else
--                             ply:ManipulateBoneScale(boneID, Vector(1, 1, 1)) -- Ensure arms/hands are visible
--                         end
--                     end
--                 end
--             end
--         elseif not isFirstPerson then
--             -- Reset manipulations if not in first person
--             g_VR.StopCharacterSystem(steamid, true) -- Call StopCharacterSystem to reset bones
--             g_VR.StartCharacterSystem(ply, true) -- Restart character system without full re-init
--         end

--         -- Original head hiding (scale only) applied in first person
--         if hideHeadSetting == 0 and isFirstPerson then
--             local charInfo = characterInfo[steamid]
--             if charInfo and charInfo.bones and charInfo.bones.b_head then
--                 ply:ManipulateBoneScale(charInfo.bones.b_head, zeroVec)
--             end
--         elseif hideHeadSetting == 0 and not isFirstPerson then
--             -- Reset scale if not first person
--             local charInfo = characterInfo[steamid]
--             if charInfo and charInfo.bones and charInfo.bones.b_head then
--                 ply:ManipulateBoneScale(charInfo.bones.b_head, Vector(1, 1, 1))
--             end
--         end
...
--         -- Also reset all bones in case mode 2 was active
...
--     function g_VR.StartCharacterSystem(ply, skipInit)
--         local steamid = ply:SteamID()
...
--             -- Remove existing callbacks before adding new ones
...
--             if ply == LocalPlayer() then
--                 hook.Remove("VRMod_PreRender", "vrutil_hook_calcplyrenderpos") -- Ensure no duplicates
...
--     function g_VR.StopCharacterSystem(steamid, keepActive)
--         if not activePlayers[steamid] then return end
--         local ply = player.GetBySteamID(steamid)
--         if not IsValid(ply) then return end
--         if characterInfo[steamid] then
--             SafeResetBoneManipulation(ply, characterInfo[steamid].bones or {}) -- Reset bones
--             if characterInfo[steamid].boneCallback then
--                 ply:RemoveCallback("BuildBonePositions", characterInfo[steamid].boneCallback)
--                 characterInfo[steamid].boneCallback = nil -- Clear callback reference
--             end
...
--             -- Don't remove hooks if other players are active unless forced or last player
--             if not keepActive and table.Count(activePlayers) <= 1 then
--                 hook.Remove("PrePlayerDraw", "vrutil_hook_preplayerdraw")
--                 hook.Remove("PostPlayerDraw", "vrutil_hook_postplayerdraw")
--                 hook.Remove("CalcMainActivity", "vrutil_hook_calcmainactivity")
--                 hook.Remove("DoAnimationEvent", "vrutil_hook_doanimationevent")
...
-- vrmod_character_type2_lua()
-- concommand.Add(
--     "vrmod_lua_reset_character_type2",
--     function(ply, cmd, args)
--         AddCSLuaFile("vrmodunoffcial/vrmod_character_type2.lua")
--         include("vrmodunoffcial/vrmod_character_type2.lua")
--         vrmod_character_type2_lua()
--     end
-- )