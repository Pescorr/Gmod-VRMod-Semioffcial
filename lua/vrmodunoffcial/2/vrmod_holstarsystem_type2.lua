        ...
        local function equipWeaponOrEntity(leftHand)
            ...
            if i then
                ...
                if wepclass ~= "" then
                        ...
                        -- 武器の場合（HL2エンジン武器はweapons.Get()に含まれないためGetWeaponでも判定）
                        if weapons.Get(wepclass) or IsValid(LocalPlayer():GetWeapon(wepclass)) then
