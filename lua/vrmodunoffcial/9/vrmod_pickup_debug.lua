...
hook.Add("VRMod_Menu", "vrmod_pickup_debug_menutab", function(frame)
	...
	if frame.Settings02Register then
		local success = frame.Settings02Register("pickup_debug", "Pickup Table", "icon16/table.png", panel)
		if not success then
			frame.DPropertySheet:AddSheet("Pickup Table", panel, "icon16/table.png")
		end
	else
		frame.DPropertySheet:AddSheet("Pickup Table", panel, "icon16/table.png")
	end
	...
	if not ok then
		print("[VRMod] Menu hook error (vrmod_pickup_debug_menutab): " .. tostring(err))
	end
end)

-- Also add HUD toggle to Pickup Assist menu
hook.Add("VRMod_Menu", "vrmod_pickup_hud_menu", function(frame)
	if not frame.DPropertySheet then return end
