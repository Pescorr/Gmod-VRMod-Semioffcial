...
hook.Add(
	...
	function(frame)
		...
		local MenuTab15 = vgui.Create("DPanel")
		...
		resetButton.DoClick = function()
			...
			RunConsoleCommand("vrmod_pickup_beam_cone_range", "25")
			RunConsoleCommand("vrmod_pickup_beam_retry_max", "10")
			RunConsoleCommand("vrmod_pickup_beamrange", "25")
			RunConsoleCommand("vrmod_pickup_beam_damage_enable", "0")
			RunConsoleCommand("vrmod_pickup_beam_damage", "0")
			RunConsoleCommand("vrmod_unoff_pickup_hud", "0")
			-- client_weight defaults
			RunConsoleCommand("vrmod_unoff_pickup_weight_client", "0")
			RunConsoleCommand("vrmod_unoff_pickup_limit_client", "0")
			-- rotation defaults
			RunConsoleCommand("vrmod_unoff_rotate_held_enable", "1")
			RunConsoleCommand("vrmod_unoff_rotate_held_speed", "0")
		end

		-- Dual-mode registration
		if frame.Settings02Register then
			local success = frame.Settings02Register("pickup_assist", "Pickup Assist", "icon16/lightning.png", MenuTab15)
			if not success then
				frame.DPropertySheet:AddSheet("Pickup Assist", MenuTab15, "icon16/lightning.png")
			end
		else
			frame.DPropertySheet:AddSheet("Pickup Assist", MenuTab15, "icon16/lightning.png")
		end
		-- MenuTab15 (Cone Pickup Assist) End

		end) -- pcall end
		if not ok then
			print("[VRMod] Menu hook error (addsettingsvrpickupbeam): " .. tostring(err))
		end
	end
)
--------[vrmod_addmenu_vrpickup.lua]End--------
