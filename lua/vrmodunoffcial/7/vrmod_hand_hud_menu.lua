...
hook.Add(
    ...
    function(frame)
        ...
        local VRhandHudsScrollPanel = vgui.Create("DScrollPanel")
        local handHudsForm = vgui.Create("DForm", VRhandHudsScrollPanel)
        ...
        for handKey, hudSettingsData in pairs(HandHUDsData) do
            local collapsibleCategory = vgui.Create("DCollapsibleCategory", handHudsForm)
            collapsibleCategory:SetLabel(hudSettingsData.menuLabel)
            collapsibleCategory:SetExpanded(true)
            handHudsForm:AddItem(collapsibleCategory)
            local categoryContents = vgui.Create("DForm", collapsibleCategory)
        ...
        if frame.Settings02Register then
            local success = frame.Settings02Register("handhud", "VRHandHUDs", "icon16/monitor_edit.png", VRhandHudsScrollPanel)
            if not success then
                frame.DPropertySheet:AddSheet("VRHandHUDs", VRhandHudsScrollPanel, "icon16/monitor_edit.png")
            end
        else
            frame.DPropertySheet:AddSheet("VRHandHUDs", VRhandHudsScrollPanel, "icon16/monitor_edit.png")
        end
        ...
        if not ok then
            print("[VRMod] Menu hook error (addsettings_handhud): " .. tostring(err))
        end
    end
)
