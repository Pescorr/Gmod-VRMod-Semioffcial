...
-- Failsafe state
local _handHUD_errorCount = 0
local _handHUD_errorThreshold = 10
local _handHUD_autoDisabled = false
local _handHUD_renderHookActive = false
local HOOK_RENDER = "VRMod_HandHUD_RenderMeshes"
...
local function RenderAllHandHUDMeshes(bDrawingDepth, bDrawingSkybox)
    if bDrawingSkybox then return end
    if _handHUD_autoDisabled then return end
    if not g_VR or not g_VR.active then return end

    for _, hudData in pairs(HandHUDs) do
        if hudData.convarValues[hudData.ID_PREFIX .. "_enabled"]
        and hudData.currentMesh and IsValid(hudData.currentMesh) then
            ...
            if not ok then
                pcall(cam.PopModelMatrix)
                pcall(cam.IgnoreZ, false)

                _handHUD_errorCount = _handHUD_errorCount + 1
                if _handHUD_errorCount <= 3 then
                    ErrorNoHaltWithStack("[VR Hand HUD] Draw error (" .. _handHUD_errorCount .. "): " .. tostring(err))
                end
                if _handHUD_errorCount >= _handHUD_errorThreshold then
                    _handHUD_autoDisabled = true
                    ErrorNoHalt("[VR Hand HUD] Auto-disabled (" .. _handHUD_errorCount .. " errors). 'vrmod_handhud_reset' to re-enable.\n")
                    hook.Remove("PostDrawTranslucentRenderables", HOOK_RENDER)
                    _handHUD_renderHookActive = false
                    return
                end
            else
                _handHUD_errorCount = 0
            end
        end
    end
end
...
local function UpdateHandHUDTransform(hudData)
    ...
    local poseFunc = vrmod[hudData.poseFuncName]
    if not poseFunc then return end
    local handPos, handAng = poseFunc()
...
local function ConfigureHandHUD(hudData)
    ...
    if anyHandHUDNowEnabled then
        ...
        if not _handHUD_renderHookActive and not _handHUD_autoDisabled then
            hook.Add("PostDrawTranslucentRenderables", HOOK_RENDER, RenderAllHandHUDMeshes)
            _handHUD_renderHookActive = true
        end
    else
...
-- Deferred: vrmod.AddCallbackedConvar may not exist yet if this addon
-- loads before the VRMod core autorun (alphabetical addon order).
timer.Simple(0, function()
    if not vrmod.AddCallbackedConvar then
        ErrorNoHalt("[vrmod_hand_hud] vrmod.AddCallbackedConvar not found - VRMod core not loaded?\n")
        return
    end
    for _, hudData in pairs(HandHUDs) do
...
hook.Add(
    ...
    function(ply)
        ...
        for _, hudData in pairs(HandHUDs) do
            ...
            -- メッシュキャッシュのクリア（メモリリーク防止）
            for k in pairs(hudData.meshesCache) do
                hudData.meshesCache[k] = nil
            end
            hudData.currentMesh = nil
        end
        ...
        hook.Remove("VRMod_PreRender", "AllHandHUDs_PreRender")
        hook.Remove("PostDrawTranslucentRenderables", HOOK_RENDER)
        hook.Remove("Think", "PeriodicHandHUDReinit")
        hook.Remove("PlayerSpawn", "HandHUD_PlayerSpawnReinit")

        -- フェールセーフリセット
        _handHUD_renderHookActive = false
        _handHUD_errorCount = 0
        _handHUD_autoDisabled = false
    end
)
...
hook.Add(
    ...
    function()
        ...
        -- vrmod.GetConvars()経由で状態を取得（毎フレームGetConVar()禁止）
        local _, globalConvarValues = vrmod.GetConvars()
        if not globalConvarValues then return end

        local leftEnabled = globalConvarValues["vrmod_left_hud_enabled_val"]
        local rightEnabled = globalConvarValues["vrmod_right_hud_enabled_val"]
...
concommand.Add("vrmod_handhud_reset", function()
    _handHUD_errorCount = 0
    _handHUD_autoDisabled = false
    if g_VR and g_VR.active then
        InitializeAllHandHUDs()
    end
    print("[VR Hand HUD] Reset. Error counter cleared.")
end)
