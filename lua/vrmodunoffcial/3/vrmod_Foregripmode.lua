...
hook.Add("VRMod_Input", "VRForegripmodeInput", function(action, pressed)
    if not gripenable:GetBool() then return end

    local leftPrimaryMode = left_primary:GetInt()
    local leftGrabMode = left_grab:GetInt()

    if leftPrimaryMode ~= 0 and action == "boolean_left_primaryfire" then
    ...
    if leftGrabMode ~= 0 and action == "boolean_left_pickup" then
