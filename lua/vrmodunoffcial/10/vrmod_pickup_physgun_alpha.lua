...
-- -- --[physgunlike-backup VRPickupSystem]start----

    -- -- 		function vrmod_pickup_physgun_lua()
    -- -- 			g_VR = g_VR or {}
    -- -- 			vrmod = vrmod or {}
    ...
    -- -- 					if entity:IsPlayer() then return end --or  --    entity:GetClass() == "worldspawn" or  --    string.sub(entity:GetClass(), 1, 5) == "func_" or --    not IsValid(entity:GetPhysicsObject()) or  --    not entity:GetPhysicsObject():IsMoveable() or --    entity:GetPhysicsObject():GetMass() > 500 or --    entity:GetPhysicsObject():HasGameFlag(FVPHYSICS_MULTIOBJECT_ENTITY) or --    (entity.CPPICanPickup and not entity:CPPICanPickup(ply)) 
    ...
    -- -- 							local handPos, handAng = LocalToWorld(handName == "left" and frame.lefthandPos or frame.righthandPos, handName == "left" and frame.lefthandAng or frame.righthandAng, t.ply:GetPos(), Angle())
    ...
    -- -- 						if controller.pickupList[k].ent == entity then return end -- Already holding with this hand
    -- -- 					end
    ...
    -- -- 		vrmod_pickup_physgun_lua()
    -- -- 		concommand.Add(
    -- -- 			"vrmod_lua_reset_pickup_physgun",
...
-- -- --[physgunlike-backup VRPickupSystem]end----




-- ----[physgunlikeV2 VRPickupSystem]start----
    -- 	function vrmod_pickup_physgun_lua()
    -- 		g_VR = g_VR or {}
    -- 		vrmod = vrmod or {}
            
    -- 		-- ConVars for beam and damage settings
    -- 		CreateClientConVar("vrmod_pickup_beam2_enable", "1", true, FCVAR_ARCHIVE, "Enable/disable the pickup beam laser")
    -- 		local maxRange = CreateClientConVar("vrmod_pickup_beam2_range", "4000", true, FCVAR_ARCHIVE, "Range of the pickup beam")
    -- 		CreateClientConVar("vrmod_pickup_beam2_damage", "0.0001", true, FCVAR_ARCHIVE, "Damage dealt by the beam laser", 0, 1.000)
    ...
    -- 			-- Store last hit positions for both hands
    -- 			local lastHitPos = {
    -- 				left = Vector(0, 0, 0),
    -- 				right = Vector(0, 0, 0)
    -- 			}
    ...
    -- 				-- Send the last hit position for more accurate grab point
    -- 				local hitPos = bLeftHand and lastHitPos.left or lastHitPos.right
    ...
    -- 			-- Function to draw beam and get hit position
    -- 			local function DrawBeamAndGetHitPos(handName)
    -- 				if not g_VR.active then return Vector(0, 0, 0) end
                    
    -- 				local startPos = g_VR.tracking["pose_" .. handName .. "hand"].pos
    -- 				local forward = g_VR.tracking["pose_" .. handName .. "hand"].ang:Forward()
    -- 				local beamRange = GetConVar("vrmod_pickup_beam2_range"):GetFloat()
                    
    -- 				-- Trace to find hit entity
    -- 				local tr = util.TraceLine({
    -- 					start = startPos,
    ...
    -- 				local heldEnt = g_VR[handName == "left" and "heldEntityLeft" or "heldEntityRight"]
                    
    -- 				-- Determine end position
    -- 				local endPos = tr.HitPos
                    
    -- 				-- Beam color
    -- 				local color = Color(0, 30, 255, 0)
    -- 				if IsValid(heldEnt) then
    -- 					color = Color(0, 255, 200, 0) -- Different color when holding
    -- 				end
                    
    -- 				-- Only draw and send damage if beam visualization is enabled
    -- 				if GetConVar("vrmod_pickup_beam2_enable"):GetBool() then
    -- 					-- Draw main beam
    -- 					render.SetMaterial(beam_mat1)
    -- 					render.DrawBeam(startPos, endPos, 2, 0, 1, color)
    ...
    -- 					-- Draw glow effects at start and end
    -- 					render.SetMaterial(beam_glow1)
    -- 					local startSize = math.random(20, 30)
    -- 					render.DrawSprite(startPos, startSize, startSize, color)
                        
    -- 					local endSize = heldEnt and IsValid(heldEnt) and math.random(10, 15) or math.random(3, 8)
    -- 					render.DrawSprite(endPos, endSize, endSize, color)
                        
    -- 					-- If we hit something, send damage event
    -- 					if tr.Hit and tr.Entity and IsValid(tr.Entity) and GetConVar("vrmod_pickup_beam2_enable"):GetBool() then
    -- 						if not IsValid(heldEnt) or tr.Entity ~= heldEnt then
    -- 							net.Start("vrmod_pickup_beam2_damage")
    -- 							net.WriteVector(tr.HitPos)
    -- 							net.WriteFloat(GetConVar("vrmod_pickup_beam2_damage"):GetFloat())
    -- 							net.SendToServer()
    -- 						end
    -- 					end
    -- 				end
                    
    -- 				-- Store hit position for pickup
    -- 				lastHitPos[handName] = tr.HitPos
                    
    -- 				return tr.HitPos
    -- 			end

    -- 			-- Add hook to draw beams for both hands
    -- 			hook.Add("PostDrawTranslucentRenderables", "vrmod_physgun_beams", function(depth, sky)
    -- 				if depth or sky then return end
                    
    -- 				DrawBeamAndGetHitPos("left")
    -- 				DrawBeamAndGetHitPos("right")
    -- 			end)
    ...
    -- 		elseif SERVER then
    -- 			util.AddNetworkString("vrmod_pickup")
    -- 			util.AddNetworkString("vrmod_pickup_beam2_damage")
    ...
    -- 			-- Handle beam damage
    -- 			net.Receive("vrmod_pickup_beam2_damage", function(len, ply)
    -- 				if not IsValid(ply) or ply:InVehicle() then return end
                    
    -- 				local hitPos = net.ReadVector()
    -- 				local damage = net.ReadFloat()
                    
    -- 				-- Apply minimal damage at the hit point to set grab point
    ...
    -- 				-- Small radius damage to affect the entity
    ...
    -- 			-- Pickup entity function
    -- 			local function pickup(handName, ply, handPos, handAng, hitPos)
    -- 				local steamid = ply:SteamID()
    -- 				local controller = PhysgunControllers[handName]
    ...
    -- 				local tr = util.TraceLine({
    -- 					start = handPos,
    -- 					endpos = handPos + forward * maxRange:GetFloat(),
    -- 					filter = ply
    -- 				})
                    
    -- 				-- Use hitPos if provided
    -- 				local targetPos = hitPos or tr.HitPos
                    
    -- 				-- Find nearest entity to target position
    -- 				local nearestEnt = nil
    -- 				local nearestDist = 10 -- Range to search for entities
                    
    -- 				for _, ent in ipairs(ents.FindInSphere(targetPos, nearestDist)) do
    -- 					if not IsValid(ent) then continue end
    -- 					if ent:IsPlayer() then continue end
    -- 					if not IsValid(ent:GetPhysicsObject()) then continue end
    -- 					--if not ent:GetPhysicsObject():IsMoveable() then continue end
                        
    -- 					local dist = targetPos:Distance(ent:GetPos())
    ...
    -- 				-- If no entity found, use trace result
    -- 				if not nearestEnt and tr.Hit and IsValid(tr.Entity) and 
    -- 				tr.Entity:GetPhysicsObject() and tr.Entity:GetPhysicsObject():IsMoveable() then
    -- 					nearestEnt = tr.Entity
    -- 				end
                    
    -- 				-- Check if we have a valid entity
    -- 				if not nearestEnt then return end
                    
    -- 				-- Check if the hook allows pickup
    -- 				if hook.Call("VRMod_Pickup", nil, ply, nearestEnt) == false then return end
    ...
    -- 							for i = 1, PhysgunControllers.left.pickupCount do
    -- 								local t = PhysgunControllers.left.pickupList[i]
    -- 								if not IsValid(t.phys) or not t.phys:IsMoveable() or 
    -- 								not g_VR[t.steamid] or not t.ply:Alive() or t.ply:InVehicle() then
    -- 									drop("left", t.steamid)
    -- 								end
    -- 							end
    ...
    -- 				-- Check if entity is already held by this hand
    -- 				for k = 1, controller.pickupCount do
    -- 					if controller.pickupList[k].ent == nearestEnt then return end -- Already holding with this hand
    -- 				end
    ...
    -- 				for k = 1, otherController.pickupCount do
    -- 					if otherController.pickupList[k].ent == nearestEnt then
    -- 						-- Entity is held by other hand, release it first
    -- 						drop(otherHandName, steamid)
    -- 						break
    -- 					end
    ...
    -- 				-- Get physics object
    -- 				local phys = nearestEnt:GetPhysicsObject()
                    
    -- 				-- Calculate grab point
    -- 				local grabPoint = targetPos
                    
    -- 				-- Calculate local position and angle relative to hand
    -- 				local entityPos = nearestEnt:GetPos()
    -- 				local entityAng = nearestEnt:GetAngles()
                    
    -- 				-- Calculate local position relative to entity, then transform to hand space
    -- 				local grabLocalPos = nearestEnt:WorldToLocal(grabPoint)
    -- 				local localPos, localAng = WorldToLocal(entityPos, entityAng, handPos, handAng)
    ...
    -- 					localPos = localPos,
    -- 					localAng = localAng,
    -- 					grabPoint = grabLocalPos,
    -- 					collisionGroup = nearestEnt:GetCollisionGroup(),
    -- 					steamid = steamid,
    -- 					ply = ply
    -- 				}
    ...
    -- 				-- Store reference on entity
    -- 				nearestEnt.vrmod_pickup_info = controller.pickupList[index]
                    
    -- 				-- Set collision group to prevent player collision
    -- 				nearestEnt:SetCollisionGroup(COLLISION_GROUP_PASSABLE_DOOR)
    ...
    -- 			-- Network receive function for pickup/drop actions
    -- 			vrmod.NetReceiveLimited("vrmod_pickup", 10, 500, function(len, ply)
    -- 				local bLeftHand = net.ReadBool()
    -- 				local bDrop = net.ReadBool()
    ...
    -- 				if not bDrop then
    -- 					pickup(handName, ply, handPos, handAng, hitPos)
    -- 				else
    -- 					local handVel = net.ReadVector() 
    -- 					local handAngVel = net.ReadVector()
    -- 					drop(handName, ply:SteamID(), handPos, handAng, handVel, handAngVel)
    -- 				end
    -- 			end)
    ...
    -- 	concommand.Add("vrmod_lua_reset_pickup_physgun", function(ply, cmd, args)
    -- 		vrmod_pickup_physgun_lua()
    -- 	end)
-- ----[physgunlikeV2 VRPickupSystem]end----
