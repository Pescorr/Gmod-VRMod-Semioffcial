-- --[[
--     VRMod Locomotion Integration
-- ]]
-- --
-- -- Shared ConVars
-- local cv_allowtp = CreateConVar("vrmod_allow_teleport", 1, {FCVAR_REPLICATED}, "Allow teleportation")
-- local cv_usetp = CreateClientConVar("vrmod_allow_teleport_client", 0 ,true, {FCVAR_ARCHIVE}, "Use teleportation")
-- -- Client ConVars
-- local cv_vehicle_steer_source = CreateClientConVar("vrmod_vehicle_steer_source",0,true, {FCVAR_ARCHIVE}, "Vehicle steer source (0: Analog stick, 1: Right hand, 2: Left hand)")
-- local cv_weapon_aim_source = CreateClientConVar("vrmod_weapon_aim_source",0,true, {FCVAR_ARCHIVE}, "Weapon aim source (0: HMD, 1: Right hand, 2: Left hand)")
-- local cv_analog_move_only = CreateClientConVar("vrmod_analog_move_only",0,true, {FCVAR_ARCHIVE}, "Use analog stick only for movement")
-- local cv_car_gun_mode = CreateClientConVar("vrmod_vehicle_reticle_mode",1,true, {FCVAR_ARCHIVE}, "Vehicle reticle mode")
-- local cv_jump_duck = CreateClientConVar("vrmod_auto_jump_duck",1,true, {FCVAR_ARCHIVE}, "Automatic jump and duck")
...
-- 			local cv_lefthandle = CreateClientConVar("vrmod_test_lefthandle", 0, false, FCVAR_ARCHIVE)
-- 			--vehicle behaviour
-- 			local vehicleSteerSource = cv_vehicle_steer_source:GetInt()
-- 			local weaponAimSource = cv_weapon_aim_source:GetInt()
-- 			local analogMoveOnly = cv_analog_move_only:GetBool()
-- 			local carGunMode = cv_car_gun_mode:GetBool()
-- 			local jumpDuck = cv_jump_duck:GetBool()
-- 			if ply:InVehicle() and not cv_cargunmode:GetBool() then
-- 				local vehicleAngles = ply:GetVehicle():GetAngles()
...
-- 				local _, relativeAng
-- 				if weaponAimSource == 1 then
-- 					_, relativeAng = WorldToLocal(Vector(0, 0, 0), g_VR.tracking.pose_righthand.ang, Vector(0, 0, 0), vehicleAngles)
-- 				elseif weaponAimSource == 2 then
-- 					_, relativeAng = WorldToLocal(Vector(0, 0, 0), g_VR.tracking.pose_lefthand.ang, Vector(0, 0, 0), vehicleAngles)
-- 				else
-- 					_, relativeAng = WorldToLocal(Vector(0, 0, 0), g_VR.tracking.hmd.ang, Vector(0, 0, 0), vehicleAngles)
-- 				end
...
-- 			local viewAngles
-- 			if weaponAimSource == 1 then
-- 				viewAngles = g_VR.currentvmi and g_VR.currentvmi.wrongMuzzleAng and g_VR.tracking.pose_righthand.ang or g_VR.viewModelMuzzle and g_VR.viewModelMuzzle.Ang or g_VR.tracking.pose_righthand.ang
-- 			elseif weaponAimSource == 2 then
-- 				viewAngles = g_VR.currentvmi and g_VR.currentvmi.wrongMuzzleAng and g_VR.tracking.pose_lefthand.ang or g_VR.viewModelMuzzle and g_VR.viewModelMuzzle.Ang or g_VR.tracking.pose_lefthand.ang
-- 			else
-- 				viewAngles = g_VR.tracking.hmd.ang
-- 			end
...
-- 				else
-- 					cmd:SetForwardMove(g_VR.input.boolean_walk and g_VR.input.vector2_walkdirection.y or 0)
-- 					cmd:SetSideMove(g_VR.input.boolean_walk and g_VR.input.vector2_walkdirection.x or 0)
-- 				end
...
-- 	0,
-- 	function()
-- 		vrmod.AddLocomotionOption("Integrated_Locomotion", start, stop, options)
-- 		vrmod.AddInGameMenuItem(
-- 			"Toggle Noclip",
-- 			2,
-- 			1,
