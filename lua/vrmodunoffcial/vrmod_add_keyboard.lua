	...
	local function ToggleKeyboard()
		...
		local function updateKeyboard()
			for i = 1, #selectedCase do
				if selectedCase[i] == "\n" then continue end
				keys[i]:SetText(selectedCase[i] == "\1" and "Back" or selectedCase[i] == "\2" and "Enter" or selectedCase[i] == "\4" and "Shift" or selectedCase[i] == "\3" and "text" or selectedCase[i])
			end
		end
		...
		for i = 1, #selectedCase do
			...
			key:SetText(selectedCase[i] == "\1" and "Back" or selectedCase[i] == "\2" and "Enter" or selectedCase[i] == "\4" and "Shift" or selectedCase[i] == "\3" and "text" or selectedCase[i])
			...
			key.OnMousePressed = function()
				if key:GetText() == "Back" then
					...
					if IsValid(activeTextEntry) then
						activeTextEntry:SetText(string.sub(activeTextEntry:GetText(), 1, #activeTextEntry:GetText() - 1))
					end
				...
				elseif key:GetText() == "Shift" then
					...
					updateKeyboard()
				elseif key:GetText() == "text" then
					VRUtilMenuClose("keyboard_only")
					LocalPlayer():ConCommand("vrmod_chatmode")
				else
					local activeTextEntry = vgui.GetKeyboardFocus()
					if IsValid(activeTextEntry) then
						activeTextEntry:SetText(activeTextEntry:GetText() .. key:GetText())
					end
				end
			end
