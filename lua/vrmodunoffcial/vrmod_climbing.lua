...
local function vrclimb()
    if SERVER then
        util.AddNetworkString("VRClimbing_Start")
        util.AddNetworkString("VRClimbing_Stop")
        local function ApplyClimbingForce(ply, handPos, climbPos, bone)
            local phys = ply:GetPhysicsObject()
            if not IsValid(phys) then return end
            local force = (climbPos - handPos) * 1000 -- 力の強さを調整
            local maxForce = 500000 -- 最大力を制限
            if force:Length() > maxForce then
                force = force:GetNormalized() * maxForce
            end

            phys:ApplyForceOffset(force, handPos)
        end
        ...
        net.Receive(
            ...
            function(len, ply)
                ...
                ply:SetNWBool("VRClimbing_" .. hand, true)
                ply:SetNWVector("VRClimbPos_" .. hand, climbPos)
                hook.Add(
                    "Think",
                    "VRClimbing_" .. ply:EntIndex() .. "_" .. hand,
                    function()
                        if not IsValid(ply) or not ply:GetNWBool("VRClimbing_" .. hand) then
                            hook.Remove("Think", "VRClimbing_" .. ply:EntIndex() .. "_" .. hand)

                            return
                        end

                        local currentHandPos = ply:GetNWVector("VRHandPos_" .. hand, Vector())
                        local climbPos = ply:GetNWVector("VRClimbPos_" .. hand)
                        ApplyClimbingForce(ply, currentHandPos, climbPos, ply:LookupBone(hand == "left" and "ValveBiped.Bip01_L_Hand" or "ValveBiped.Bip01_R_Hand"))
                    end
                )
            end
        )
        ...
        net.Receive(
            ...
            function(len, ply)
                local hand = net.ReadString()
                ply:SetNWBool("VRClimbing_" .. hand, false)
                hook.Remove("Think", "VRClimbing_" .. ply:EntIndex() .. "_" .. hand)
            end
        )
    end
    ...
    if CLIENT then
        local HAND_TRACE_DISTANCE = 100
        local function StartClimbing(hand)
            local handPos = g_VR.tracking["pose_" .. hand .. "hand"].pos
            local handAng = g_VR.tracking["pose_" .. hand .. "hand"].ang
            local traceResult = util.TraceLine(
                {
                    start = handPos,
                    endpos = handPos + handAng:Forward() * HAND_TRACE_DISTANCE,
                    filter = LocalPlayer()
                }
            )

            if traceResult.Hit and (traceResult.HitWorld or (IsValid(traceResult.Entity) and traceResult.Entity:GetClass() == "prop_physics")) then
                net.Start("VRClimbing_Start")
                net.WriteString(hand)
                net.WriteVector(handPos)
                net.WriteVector(traceResult.HitPos)
                net.SendToServer()
                LocalPlayer():EmitSound("physics/concrete/concrete_impact_soft1.wav", 75, 100, 0.5)
            end
        end

        local function StopClimbing(hand)
            net.Start("VRClimbing_Stop")
            net.WriteString(hand)
            net.SendToServer()
            LocalPlayer():EmitSound("physics/concrete/concrete_impact_soft2.wav", 75, 100, 0.5)
        end
        ...
        hook.Add(
            "Think",
            "VRClimbing_UpdateHandPos",
            function()
                if not g_VR.active then return end
                for _, hand in ipairs({"left", "right"}) do
                    local handPos = g_VR.tracking["pose_" .. hand .. "hand"].pos
                    LocalPlayer():SetNWVector("VRHandPos_" .. hand, handPos)
                end
            end
        )
        ...
        hook.Add(
            ...
            function()
                if not g_VR.active then return end
                for _, hand in ipairs({"left", "right"}) do
                    if LocalPlayer():GetNWBool("VRClimbing_" .. hand) then
                        local handPos = g_VR.tracking["pose_" .. hand .. "hand"].pos
                        local climbPos = LocalPlayer():GetNWVector("VRClimbPos_" .. hand)
                        render.DrawLine(handPos, climbPos, Color(0, 255, 0), true)
                        render.DrawWireframeSphere(climbPos, 5, 10, 10, Color(255, 0, 0), true)
                    end
                end
            end
        )
