local cvar_mass = CreateConVar("vrmod_pickup_weight","35",bit.bor(FCVAR_REPLICATED),"Highest mass that can be picked up, in kilograms",0)
local cvar_range = CreateConVar("vrmod_pickup_range","72",bit.bor(FCVAR_REPLICATED),"How far you can reach",0)
...
local function CanGrab(e,phys,pl)
	...
	return phys:IsValid() && phys:GetMass() <= cvar_mass:GetFloat()
end
...
if CLIENT then
	...
	function vrmod.PickupPlus( bLeftHand, bDrop )
		net.Start("vrmod_pickup_plus")
	...
	function vrmod.OnPickup(entid,pl,bLeft,localPos,localAng,createtime)
		...
		if ent:IsValid() && ent:GetCreationTime() == createtime then
			...
			hook.Run("vrmod_pickup_plus", pl, ent)
		end
	end
	...
	net.Receive("vrmod_pickup_plus",function(len)
		...
		if vrmod.InEye(true) then
			for i = 1,2 do
				...
				for _,t in ipairs(GetGrabbableObjects(pl,pickupPoint,ang)) do
					if hook.Run("vrmod_pickup_plus", pl, t.ent) == false then continue end
	...
	hook.Add("VRMod_Start","VRMod_InitHalos",function(pl)
		if pl == LocalPlayer() then
			hook.Add("PreDrawHalos","vrmod_pickup_plusHalos",g_VR.DrawPickupHalos)
		end
	end)

	hook.Add("VRMod_Exit","VRMod_RemoveHalos",function(pl)
		if pl == LocalPlayer() then
			hook.Remove("PreDrawHalos","vrmod_pickup_plusHalos")
		end
	end)]]

elseif SERVER then

	util.AddNetworkString("vrmod_pickup_plus")
	...
	local function CreatePhysHook()
		...
		hook.Add("Tick","vrmod_pickup_plus",function()
	...
	local function RemovePhysHook()
		hook.Remove("Tick","vrmod_pickup_plus")
	end
	...
	function vrmod.AttemptPickup(ply, bLeftHand, handPos, handAng)
		...
		for _,t in ipairs(GetGrabbableObjects(ply,pickupPoint,handAng)) do
			if hook.Run("VRMod_Pickup_plus", ply, t.ent) == false then continue end
	...
	vrmod.NetReceiveLimited("vrmod_pickup_plus",10,400,function(len, ply)
