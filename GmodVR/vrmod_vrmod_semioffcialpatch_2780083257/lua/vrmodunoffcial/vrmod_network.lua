...
if CLIENT then
			...
			hook.Add(
				...
				function()
					...
					for k, v in pairs(g_VR.net) do
						if not v.playbackTime then continue end
	...
	--called by localplayer when they enter vr
	function VRUtilNetworkInit()
		-- transmit loop
		timer.Create(
			"vrmod_transmit",
			1 / convarValues.vrmod_net_tickrate,
			0,
			function()
	...
	local function LerpOtherVRPlayers()
		...
		for k, v in pairs(g_VR.net) do
			...
			if #v.frames < 2 or not ply then continue end --len check discards the localplayer, ply might be invalid for a few frames before exit msg upon disconnect
			if v.buffering then
	...
	--called by localplayer when they exit vr
...
elseif SERVER then
	...
	vrmod.NetReceiveLimited(
		"vrutil_net_tick",
		convarValues.vrmod_net_tickrate + 5,
		1200,
		function(len, ply)
	...
	hook.Add(
		"PlayerDisconnected",
		"vrutil_hook_playerdisconnected",
		function(ply)
			net_exit(ply:SteamID())
		end
	)
	...
	hook.Add(
		"PlayerSwitchWeapon",
		"vrutil_hook_playerswitchweapon",
		function(ply, old, new)
			if g_VR[ply:SteamID()] ~= nil then
				...
				net.Send(ply)
				timer.Simple(0, function() end) --new:AddEffects(EF_NODRAW) 
			end
		end
	)
	...
	hook.Add(
		...
		"vrutil_hook_playerenteredvehicle",
		function(ply, veh)
			local emptyhandrepl = CreateConVar("vrmod_emptyhanded_swep", "weapon_vrmod_empty", FCVAR_ARCHIVE + FCVAR_REPLICATED)
			if g_VR[ply:SteamID()] ~= nil then
				ply:SelectWeapon(emptyhandrepl:GetString())
	...
	hook.Add(
		"PlayerLeaveVehicle",
		"vrutil_hook_playerleavevehicle",
		function(ply, veh)
			if g_VR[ply:SteamID()] ~= nil then
