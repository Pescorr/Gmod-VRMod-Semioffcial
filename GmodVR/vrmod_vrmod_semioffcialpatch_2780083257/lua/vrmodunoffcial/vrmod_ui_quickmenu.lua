...
function g_VR.MenuOpen()
	...
	else -- -- -- -- elseif mode == 4 then -- local newpos = pos -- local newang = ang -- VRUtilMenuOpen("miscmenu",512, 512, nil, 4, newpos, newang, 0.03, true, function() --forw, left, up -- hook.Remove("PreRender","vrutil_hook_renderigm") -- open = false -- if items[prevHoveredItem] and g_VR.menuItems[items[prevHoveredItem].index] then -- g_VR.menuItems[items[prevHoveredItem].index].func() -- end -- end) -- --
		VRUtilMenuOpen(
			"miscmenu",
			512,
			512,
