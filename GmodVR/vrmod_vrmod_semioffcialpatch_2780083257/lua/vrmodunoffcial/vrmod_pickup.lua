...
vrmod.AddCallbackedConvar("vrmod_pickup_range", nil, 75,  FCVAR_REPLICATED + FCVAR_ARCHIVE, "",1.0, 99999, tonumber)--cvarName, valueName, defaultValue, flags, helptext, min, max, conversionFunc, callbackFunc
...
if not !GetConVar("vrgrab_range") then
	...
	if CLIENT then
		...
		net.Receive("vrmod_pickup_legacy",function(len)
			...
			if bDrop then
				...
				--]]
				hook.Call("VRMod_Pickup_legacy", nil, ply, ent)
	...
	if SERVER then
		...
		util.AddNetworkString("vrmod_pickup_legacy")
		...
		local function drop(steamid, bLeftHand, handPos, handAng, handVel, handAngVel)
			for i = 1,pickupCount do
				...
				if pickupCount == 0 then
					...
					hook.Remove("Tick","vrmod_pickup_legacy")
		...
		local function pickup(ply, bLeftHand, handPos, handAng)
			...
			for k = 1,#entities do local v = entities[k]
				...
				--pescorrzoneend
				if not WorldToLocal(pickupPoint - v:GetPos(), Angle(), Vector(), v:GetAngles()):WithinAABox(v:OBBMins()*convarValues.vrmod_pickup_range/70, v:OBBMaxs()*convarValues.vrmod_pickup_range/70) then continue end
				if hook.Call("VRMod_Pickup_legacy", nil, ply, v) == false then return end
				--
				if pickupController == nil then
					--print("created controller")
					pickupController = ents.Create("vrmod_pickup_legacy")
					...
					pickupController:StartMotionController()
						hook.Add("Tick","vrmod_pickup_legacy",function()
			...
			vrmod.NetReceiveLimited("vrmod_pickup_legacy",10,400,function(len, ply)
				local bLeftHand = net.ReadBool()
				local bDrop = net.ReadBool()
				if not bDrop then
